...
my $supress_error_output = 0;
my $debug_ktool          = 0;
my $print_full_output    = 0;
my $output_file          = "-";    #set stdout by default
...
sub build_and_parse_kconfig_tree {
    ...
    while ( ( @parseline = shift(@config_to_parse) ) && ( exists $parseline[0]{text} ) ) {
        ...
        #prompt <prompt> [if <expr>]
            ...
            $line =~ /^\s*prompt\s+"?([^"]+)"?/;
            handle_prompt( $1, $inside_config, $inside_choice, $filename, $line_no );
        }
...
sub handle_expressions {
    my ( $exprline, $inside_config, $filename, $line_no, $inside_expression ) = @_;

    return unless ($exprline);

    unless (defined $inside_expression) {
        #do something here
        ...
        if ($inside_help) {
            ...
            if ( ( $line !~ /$help_whitespace/ ) && ( $line !~ /^[\r\n]+/ ) ) {
                ...
                if ($inside_config) {
                    ...
                    my @help_array = @{$symbols{$inside_config}{$sym_num}{helptext}};
                    push( @help_array , $line );
                }
            }
        }
            ...
            if ( ( !$inside_config ) && ( !$inside_choice ) ) {
                ...
                $symbols{$inside_config}{$sym_num}{helptext}     = \@help_array;
            }
        }
        return $inside_help;
    }
